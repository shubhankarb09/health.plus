...
parse_str($clinicid, $output);
$parsedid=$output['ID'];
$getappointmentsquery= "SELECT * FROM Appointment WHERE clinicid='$parsedid'";
...
for($i=0;$i<count($pidstack);$i++){
    ...
    echo "<a href='clinic_search_detail.php?pname=$pnamerow[0]&cname=$cnamerow[0]&ename=$enamerow[0]
    &bookdate=$bookdatestack[$i]&timeofday=$timeofdaystack[$i]&assigneddoctor=$assigneddoctorstack[$i]'>".
    $pnamerow[0]."<br>".$cnamerow[0]."<br>".$enamerow[0]."<br>".$bookdatestack[$i]."<br>".$timeofdaystack[$i]."<br>".
